...
void GlobalOptimization::XYZ2GPS(double *lla, double x, double y, double z)
{
//    if(!initGPS)
//    {
//        geoConverter.Reset(latitude, longitude, altitude);
...
//    printf("new gps: t: %f lat: %f lon: %f alt:%f ", t, latitude, longitude, altitude);
//    printf("------>>>>>>  x: %f y: %f z:%f \n", t, tmp[0], tmp[1], tmp[2]);
...
void GlobalOptimization::output_global_pose()
{
    std::ofstream foutC("/home/myk/slam_lrn/output/final_global.txt", ios::app);//app會在打開時清空
    foutC.setf(ios::fixed, ios::floatfield);
    map<double, vector<double>>::iterator iter;
    for (iter = globalPoseMap.begin(); iter != globalPoseMap.end(); iter++) {
        foutC.precision(0);
        foutC << iter->first * 1e9 << " ";
        foutC.precision(6);
        foutC << iter->second[0] << " "
              << iter->second[1] << " "
              << iter->second[2] << " "
...
//输出RTK真值
void GlobalOptimization::output_global_truth()
{
    std::ofstream foutC("/home/myk/slam_lrn/output/global_truth.txt", ios::app);//app會在打開時清空
    foutC.setf(ios::fixed, ios::floatfield);
    map<double, vector<double>>::iterator iter;
//    map<double, vector<double>>::iterator iter_2;
    for (iter = GPSPositionMap.begin(); iter != GPSPositionMap.end(); iter++) {
        ...
        foutC << iter->second[0] << " "
              << iter->second[1] << " "
              << iter->second[2] << " "
              << globalPoseMap.begin()->second[4] << " "
              << globalPoseMap.begin()->second[5] << " "
              << globalPoseMap.begin()->second[6] << " "
              << globalPoseMap.begin()->second[3] << endl;
    }
    foutC.close();
}
//path"
